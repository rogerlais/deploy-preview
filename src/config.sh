...
#* Criar os volumes com seus compartilhamentos primários( o volume sistema foi criado acima da entrada no domínio)
create_volumes success "$(get_volume_config_file)" 0 #0/null -> sem limite
if [[ $success -ne 0 ]]; then
    echo "Falha criando os volumes no pool de armazenamento primário!!!" | slog 3
    exit
else
...
#*Ajustando as permissões para os compartilhamentos
